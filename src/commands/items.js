...
export function buildVocalLevelCommand(options = {}) {
  ...
  return {
    ...
    levelMode: parseLevelMode(options.levelMode ?? options["level-mode"] ?? "relative"),
    ...
    gainDeadbandDb: parseNumberOption(options.gainDeadbandDb ?? options["gain-deadband-db"] ?? 1.5, "Gain deadband dB", 0, 12),
    ...
    detectWindowMs: parseNumberOption(options.detectWindowMs ?? options["detect-window-ms"] ?? options.gateWindowMs ?? options["gate-window-ms"] ?? 10, "Detection window length ms", 10, 500),
    detectSilenceDb: parseNumberOption(options.detectSilenceDb ?? options["detect-silence-db"] ?? -50, "Detection silence threshold dBFS", -120, -20),
    detectRangeDb: parseNumberOption(options.detectRangeDb ?? options["detect-range-db"] ?? options.gateRangeDb ?? options["gate-range-db"] ?? 45, "Detection range dB", 6, 100),
    partMergeGapMs: parseNumberOption(options.partMergeGapMs ?? options["part-merge-gap-ms"] ?? options.activationMergeGapMs ?? options["activation-merge-gap-ms"] ?? 140, "Part merge gap ms", 0, 3000),
    minPartMs: parseNumberOption(options.minPartMs ?? options["min-part-ms"] ?? options.minActivationMs ?? options["min-activation-ms"] ?? options.minPhraseMs ?? options["min-phrase-ms"] ?? 90, "Minimum part length ms", 50, 10000),
    syllableSplitDb: parseNumberOption(options.syllableSplitDb ?? options["syllable-split-db"] ?? 10, "Syllable split dB", 0, 40),
    syllableSplitHoldMs: parseNumberOption(options.syllableSplitHoldMs ?? options["syllable-split-hold-ms"] ?? 60, "Syllable split hold ms", 0, 1000),
    minGainChangeDb: parseNumberOption(options.minGainChangeDb ?? options["min-gain-change-db"] ?? 7, "Minimum gain change dB", 0, 24),
    ...
    curveSmoothMs: parseNumberOption(options.curveSmoothMs ?? options["curve-smooth-ms"] ?? 800, "Curve smoothing ms", 0, 1000),
    curveToleranceDb: parseNumberOption(options.curveToleranceDb ?? options["curve-tolerance-db"] ?? 7, "Curve tolerance dB", 0, 12),
    curveMinPointGapMs: parseNumberOption(options.curveMinPointGapMs ?? options["curve-min-point-gap-ms"] ?? 800, "Curve minimum point gap ms", 0, 1000),
    curveDetail: parseNumberOption(options.curveDetail ?? options["curve-detail"] ?? 0.08, "Curve detail", 0, 1),
    curveEdgeRampMs: parseNumberOption(options.curveEdgeRampMs ?? options["curve-edge-ramp-ms"] ?? 160, "Curve edge ramp ms", 0, 500),
