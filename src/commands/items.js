...
export function buildVocalLevelCommand(options = {}) {
  ...
  return {
    ...
    partMergeGapMs: parseNumberOption(options.partMergeGapMs ?? options["part-merge-gap-ms"] ?? options.activationMergeGapMs ?? options["activation-merge-gap-ms"] ?? 90, "Part merge gap ms", 0, 3000),
    minPartMs: parseNumberOption(options.minPartMs ?? options["min-part-ms"] ?? options.minActivationMs ?? options["min-activation-ms"] ?? options.minPhraseMs ?? options["min-phrase-ms"] ?? 80, "Minimum part length ms", 50, 10000),
    syllableSplitDb: parseNumberOption(options.syllableSplitDb ?? options["syllable-split-db"] ?? 8, "Syllable split dB", 0, 40),
    syllableSplitHoldMs: parseNumberOption(options.syllableSplitHoldMs ?? options["syllable-split-hold-ms"] ?? 45, "Syllable split hold ms", 0, 1000),
    minGainChangeDb: parseNumberOption(options.minGainChangeDb ?? options["min-gain-change-db"] ?? 3, "Minimum gain change dB", 0, 24),
    ...
    curveSmoothMs: parseNumberOption(options.curveSmoothMs ?? options["curve-smooth-ms"] ?? 120, "Curve smoothing ms", 0, 1000),
    curveToleranceDb: parseNumberOption(options.curveToleranceDb ?? options["curve-tolerance-db"] ?? 1.5, "Curve tolerance dB", 0, 12),
    curveMinPointGapMs: parseNumberOption(options.curveMinPointGapMs ?? options["curve-min-point-gap-ms"] ?? 90, "Curve minimum point gap ms", 0, 1000),
    curveDetail: parseNumberOption(options.curveDetail ?? options["curve-detail"] ?? 0.75, "Curve detail", 0, 1),
    curveEdgeRampMs: parseNumberOption(options.curveEdgeRampMs ?? options["curve-edge-ramp-ms"] ?? 45, "Curve edge ramp ms", 0, 500),
